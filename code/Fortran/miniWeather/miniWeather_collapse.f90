...
contains
  ...
  subroutine semi_discrete_step( state_init , state_forcing , state_out , dt , dir , flux , tend )
    ...
    !$acc parallel loop collapse(3) copyin(state_init, tend) copyout(state_out)
    do ll = 1 , NUM_VARS
      do k = 1 , nz
        do i = 1 , nx
  ...
  subroutine compute_tendencies_x(state,flux,tend)
    ...
    !$acc parallel loop collapse(3) copyout(tend) copyin(flux)
    do ll = 1 , NUM_VARS
      do k = 1 , nz
        do i = 1 , nx
  ...
  subroutine compute_tendencies_z(state,flux,tend)
    ...
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    !Use the fluxes to compute tendencies for each cell
    !$acc parallel loop collapse(3) copyout(tend) copyin(state, flux)
    do ll = 1 , NUM_VARS
      do k = 1 , nz
        do i = 1 , nx
  ...
  subroutine set_halo_values_x(state)
    ...
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    !$acc parallel loop collapse(2) copy(state)
    do ll = 1 , NUM_VARS
      do k = 1 , nz
  ...
  subroutine reductions( mass , te )
    ...
    !$acc parallel loop reduction(+:mass,te) collapse(2) copyin(hy_dens_cell, &
