...
program miniweather
  ...
  !$acc data create(tend, flux) &
  !$acc copyin(state, state_tmp, hy_dens_theta_cell, hy_dens_cell, hy_dens_int, hy_pressure_int, hy_dens_theta_int)
  ...
  !! MAIN TIME STEP LOOP
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  !$acc update device(state, state_tmp, flux, tend, hy_dens_theta_cell, hy_dens_cell, hy_dens_int, hy_pressure_int, hy_dens_theta_int)
  ...
  !$acc update host(state, hy_dens_theta_cell, hy_dens_cell)
...
contains
  ...
  subroutine semi_discrete_step( state_init , state_forcing , state_out , dt , dir , flux , tend )
    ...
    !!$acc update device(state_forcing, state_init, flux, tend, hy_dens_theta_cell, hy_dens_cell, hy_dens_int, hy_pressure_int, hy_dens_theta_int)
    ...
    !$acc parallel loop collapse(3) present(state_init, tend)
    do ll = 1 , NUM_VARS
      do k = 1 , nz
        do i = 1 , nx
   ...
   !!$acc update host(state_forcing, state_out, flux, tend, hy_dens_theta_cell, hy_dens_cell, hy_dens_int, hy_pressure_int, hy_dens_theta_int)
  ...
  subroutine compute_tendencies_x(state,flux,tend)
    ...
    !$acc parallel loop gang present(state, flux, hy_dens_theta_cell, hy_dens_cell)
    do k = 1 , nz
    ...
    !$acc parallel loop collapse(3) present(tend, flux)
    do ll = 1 , NUM_VARS
      do k = 1 , nz
        do i = 1 , nx
  ...
  subroutine compute_tendencies_z(state,flux,tend)
    ...
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    !Compute fluxes in the x-direction for each cell
    !$acc parallel loop gang present(hy_dens_int, state, hy_pressure_int, hy_dens_theta_int, flux)
    do k = 1 , nz+1
      !$acc loop vector private(stencil,vals,d3_vals)
      do i = 1 , nx
        !Use fourth-order interpolation from four cell averages to compute the value at the interface in question
    ...
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    !Use the fluxes to compute tendencies for each cell
    !$acc parallel loop collapse(3) present(tend, state, flux)
    do ll = 1 , NUM_VARS
      do k = 1 , nz
        do i = 1 , nx
  ...
  subroutine set_halo_values_x(state)
    ...
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    !$acc parallel loop collapse(2) present(state)
    do ll = 1 , NUM_VARS
      do k = 1 , nz
  ...
  subroutine reductions( mass , te )
    ...
    !$acc parallel loop collapse(2) reduction(+:mass,te) present(hy_dens_cell, &
